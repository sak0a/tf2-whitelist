...
try {
    ...
    if ($application['status'] !== 'pending') {
        $_SESSION['admin_error'] = 'Only pending applications can be quickly approved';
        header('Location: applications.php');
        exit;
    }

    // Call the quick approval procedure if available, otherwise do it directly
    if (function_exists('mysqli_connect')) {
        ...
        $stmt = $mysqli->prepare("CALL QuickApprove(?, ?, ?)");
        ...
        $stmt = $pdo->prepare("UPDATE whitelist_applications SET status = 'approved', approved_at = NOW(), updated_at = NOW() WHERE id = :id");
        $stmt->execute([':id' => $application_id]);

        // Log the action
        $stmt = $pdo->prepare("INSERT INTO activity_log (admin_id, application_id, application_status, action, details, ip_address, timestamp) 
                              VALUES (:admin_id, :app_id, 'approved', 'approve', 'Application quickly approved', :ip, NOW())");
        $stmt->execute([
            ':admin_id' => $admin_id,
    ...
    $_SESSION['admin_success'] = "Application #$application_id has been approved";

} catch (Exception $e) {
    $_SESSION['admin_error'] = "Error updating application: " . $e->getMessage();
    logMessage('admin_error.log', "Error quick approving application $application_id: " . $e->getMessage());
}
